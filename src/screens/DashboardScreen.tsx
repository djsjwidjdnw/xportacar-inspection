...
export function DashboardScreen({ navigation }: { navigation: { navigate: (s: string, p?: object) => void } }) {
  ...
  const [completed, setCompleted] = useState<AssignedVehicle[]>([]);
  ...
  const load = useCallback(async () => {
    ...
    // Vehicles I've already inspected (last 20).
    ...
    setCompleted((done as AssignedVehicle[]) ?? []);
  }, [user]);
    ...
    <View style={{ flex: 1, backgroundColor: theme.colors.bg }}>
      <View style={styles.header}>
        <View>
          ...
          <Text style={styles.subtitle}>{assigned.length} assigned · {completed.length} completed (recent)</Text>
        </View>
        ...
        renderItem={({ item }) => (
          <View style={{ marginBottom: 24 }}>
            <Text style={styles.section}>
              {item.section === "assigned" ? "Assigned to you" : "Recently completed"}
            </Text>
            {(item.section === "assigned" ? assigned : completed).length === 0 ? (
              <View style={styles.empty}>
                <Text style={styles.muted}>
                  {item.section === "assigned"
                    ? "No vehicles assigned. Ask an admin to assign one to you."
                    : "Nothing here yet — completed inspections will show up after you submit one."}
                </Text>
              </View>
            ) : (
              (item.section === "assigned" ? assigned : completed).map((v) => (
                <Pressable
                  key={v.id}
                  style={({ pressed }) => [styles.row, pressed && { opacity: 0.9 }]}
                  onPress={() => item.section === "assigned"
                    ? navigation.navigate("Inspect", { vehicleId: v.id })
                    : navigation.navigate("Inspect", { vehicleId: v.id, readOnly: true })}
                >
                  <View style={{ flex: 1 }}>
...
const styles = StyleSheet.create({
  ...
  section: { fontSize: 11, fontWeight: "800", color: theme.colors.textLight, textTransform: "uppercase", letterSpacing: 0.5, marginBottom: 8 },
